...
class BoardsListTableViewCell: UITableViewCell {
    ...
    var isChosen = false {
        didSet {
            if isChosen {
                actionImage.image = generateImage(Constants.Design.Image.iconRemoveFromFavorites)
            } else {
                actionImage.image = generateImage(Constants.Design.Image.iconFavorites)
            }
...
//        var image = Constants.Design.Image.iconFavorites.withConfiguration(configuration).withTintColor(.white, renderingMode: .alwaysOriginal)
...
//        backgroundColor = .systemBackground
    ...
    func generateImage(_ image: UIImage) -> UIImage {
//        UIImage(systemName: )
        let configuration = UIImage.SymbolConfiguration(scale: .large)
//        var image = UIImage(systemName: name, withConfiguration: configuration)!
//        return image.withConfiguration(configuration).withTintColor(#colorLiteral(red: 1, green: 1, blue: 1, alpha: 1), renderingMode: .automatic)
        return image.withConfiguration(configuration).withTintColor(.white, renderingMode: .alwaysOriginal)
...
//        return image
    ...
    override func layoutSubviews() {
        ...
        actionView.frame.size.width += actionImage.image!.size.width + 30 + 50
        
        actionImage.frame.size.width = actionImage.image!.size.width
        actionImage.frame.size.height = self.bounds.height
        actionImage.frame.origin.x = actionView.bounds.maxX - actionImage.image!.size.width - 15 - 50
    }
...
//                    actionView.backgroundColor = #colorLiteral(red: 0, green: 0.6745420694, blue: 0.2156436443, alpha: 1)
                ...
                if abs(panGesture.translation(in: self).x) >= (actionImage.image!.size.width + 30) {
                    actionView.frame.origin.x = originSaved.x - (actionImage.image!.size.width + 30)
                }
                ...
                if
                    (gestureView.frame.origin.x < CGFloat(-(actionImage.image!.size.width + 30))) &&
                    canPlayHapticFeedback
                {
                    ...
                    UIView.animate(
                        withDuration: 0.1,
                        animations: {
                            self.actionImage.transform = CGAffineTransform(scaleX: 1.3, y: 1.3)
                        },
                        completion: { isCompleted  in
                            UIView.animate(withDuration: 0.1) {
                                self.actionImage.transform = CGAffineTransform.identity
                            }
                        }
                    )
                ...
                if gestureView.frame.origin.x > CGFloat(-(actionImage.image!.size.width + 30)) {
        ...
        case .ended:
            ...
            UIView.animate(
                ...
                animations: {
                    ...
                    if abs(panGesture.translation(in: self).x) < self.actionImage.image!.size.width + 30 {
